package hw14.server.utils;

import static hw14.server.utils.HttpUrlConnectionHelper.sendRequest;
...
public final class WebServerHelper {
    ...
    public static HttpCookie login(String url, String login, String password) throws IOException {
        ...
        final HttpURLConnection connection = sendRequest(url, HttpMethod.POST);
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                return cookies.stream().filter(c -> c.getName().equalsIgnoreCase(COOKIE_NAME_JSESSIONID)).findFirst()
                        .orElse(null);
            }
        } finally {
